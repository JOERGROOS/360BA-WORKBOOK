...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (b.richtung) {
    ...
    if (eAlle) return dbFehler('kapitel', eAlle, 'Kapitel konnten nicht geladen werden.');
    if (!alle) return NextResponse.json({ error: 'Kapitel konnten nicht geladen werden.' }, { status: 500 });
  ...
  if (error) return dbFehler('kapitel', error, 'Kapitel konnte nicht gespeichert werden.');
  return NextResponse.json({ ok: true });
}
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (error) return dbFehler('kapitel', error, 'Kapitel konnte nicht gelöscht werden.');
  return NextResponse.json({ ok: true });
}
