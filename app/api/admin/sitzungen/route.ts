...
export async function GET(req: Request) {
  ...
  const { data, error } = await db.from('wb_sessions')
    .select('id,token,vorname,nachname,firma,email,status,test,created_at,abgeschlossen_at,fragen_snapshot,antworten,abholen_angefordert,termin_am,management_summary_path')
