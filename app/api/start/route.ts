...
export async function POST(req: Request) {
  ...
  if (!bremse('start:alle', 60, 3600)) return NextResponse.json({ error: 'Zu viele Versuche. Bitte später erneut.' }, { status: 429 });
  ...
  if (!bremse(`start:mail:${k.email}`, 2, 3600)) return NextResponse.json({ error: 'Zu viele Versuche. Bitte später erneut.' }, { status: 429 });
