...
export function Fortschritt({ kapitel, aktuellesKapitel, prozent }: { kapitel: { titel: string }[]; aktuellesKapitel: number; prozent: number }) {
  ...
  return (
    <div className="max-w-[1100px] mx-auto px-6 md:px-10 pt-5 flex items-center gap-4 md:gap-5">
      {/* Gesamtfortschritt als Kreis — auf dem Handy trägt ihn die Kopfzeile neben „Frage x von y". */}
      <div className="hidden md:block">
        <Ring prozent={prozent} groesse={48} strich={4}
          kinder={<span className="text-[12.5px] font-semibold tabular-nums">{prozent}<span className="text-muted"> %</span></span>} />
      </div>
