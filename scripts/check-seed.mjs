...
for (const key of ['zugang_text','einladung_titel','einladung_text','landing_titel','landing_intro','landing_video_hinweis','kacheln_titel','kachel_workbook','kachel_finanzdaten','upload_hinweis','willkommen_text','bestandteile_text','finanzcheck_liste','faktoren_einleitung','ergebnis_text','aha_frage','ueber_text','mail_link_text','mail_fertig_text']) assert.ok(t[key], `Text ${key} fehlt`);
assert.ok('video_url' in t, 'Text video_url fehlt (darf leer sein)');
assert.ok(t.landing_titel.includes('{vorname}'), 'landing_titel braucht {vorname}');
assert.ok(t.landing_titel.includes(','), 'landing_titel braucht ein Komma — danach steht der orange Teil');
console.log('ok');
